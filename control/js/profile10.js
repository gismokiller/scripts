                ...
                statusLog.innerHTML = `
                    <div style="background: var(--bg-card); border: 1px solid var(--border); border-radius: 10px; padding: 20px;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 8px;">
                            ...
                            <button onclick="handleAction('setProfilePicture', null, '${safeUrl}')" class="btn btn-primary btn-block" style="font-size: 12px;">Aplicar a esta cuenta</button>
...
function renderSessionsProfileTable(sessions) {
    ...
    sessions.forEach((s, index) => {
        ...
        const rowId = `row-profile-${index}`;
        
        html += `
        <tr id="${rowId}" onclick="openImageModal('${nombreRed.replace(/'/g, "\\'")}', '${sessionCleanName.replace(/'/g, "\\'")}', '${imagenRed.replace(/'/g, "\\'")}', '${s.name_session.replace(/'/g, "\\'")}')" style="cursor: pointer;">
            <td>
            ...
            <td style="text-align: center;" onclick="event.stopPropagation()">
                <div style="display: flex; justify-content: center; gap: 8px;">
                    <button onclick="event.stopPropagation(); openModalEditName('${s.name_session}', '${nombreRed.replace(/'/g, "\\'")}')" 
                            class="icon-btn"
                            title="Cambiar Nombre"
                            ${!isWorking ? 'style="opacity: 0.5; pointer-events: none;"' : ''}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                    </button>
                    <button onclick="event.stopPropagation(); openModalEditPhoto('${s.name_session}')" 
                            class="icon-btn"
                            title="Cambiar Foto"
                            ${!isWorking ? 'style="opacity: 0.5; pointer-events: none;"' : ''}>
...
async function confirmNameChangeEdit() {
    ...
    try {
        ...
        const res = await response.json();
        const rawMsg = res.body?.mensaje || res.mensaje || 'Nombre actualizado';
...
function openModalEditPhoto(sessionName) {
    ...
    fileInput.onchange = async (e) => {
        ...
        try {
            ...
            const rawMsg = setRes.body?.mensaje || setRes.mensaje || 'Foto actualizada';
            const msg = escapeHtml(rawMsg);
