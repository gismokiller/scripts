...
function renderSessionsProfileTable(sessions) {
    ...
    sessions.forEach(s => {
        ...
        <tr>
            <td>
                <div style="font-weight: 600;">${sessionCleanName}</div>
                <div style="color: var(--text-muted); font-size: 11px;">${s.name_session}</div>
            </td>
            <td>
            ...
            <td>
                <div style="display: flex; align-items: center; gap: 12px;">
                    <img src="${s.profilePicture || ''}" alt="" class="group-photo" style="width: 40px; height: 40px;" onerror="this.src='data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 40 40%22><rect fill=%22%231c2128%22 width=%2240%22 height=%2240%22 rx=%228%22/><text x=%2250%25%22 y=%2255%25%22 text-anchor=%22middle%22 fill=%22%236e7681%22 font-size=%2214%22>👤</text></svg>'">
                    <div>
                        <div style="font-weight: 500;">${s.profileName || 'Sin nombre'}</div>
                    </div>
                </div>
            </td>
            <td style="text-align: center;">
                <div style="display: flex; justify-content: center; gap: 8px;">
                    <button onclick="openModalEditName('${s.name_session}', '${s.profileName || ''}')" 
                            class="icon-btn"
...
async function confirmNameChangeEdit() {
    ...
    try {
        ...
        setTimeout(() => fetchAllSessionsForProfile(), 1500);
    } catch (e) {
...
function closeModalEditPhoto() {
    document.getElementById('modalPhotoEdit').classList.remove('active');
}
